...
class staffProfileForm(forms.ModelForm):
    """Full profile edit form for staff member"""
    ...
    class Meta:
        ...
        fields = [
            ...
            'nationality', 'address',
            'email', 'phone', 'personal_email',
            'emergency_contact_name', 'emergency_contact_phone',
            'reporting_manager', 'work_location', 'employment_type',
            'skills',
        ]
        widgets = {
            'profile_photo': forms.FileInput(attrs={'class': 'mp-input', 'accept': 'image/*', 'id': 'mpPhotoInput'}),
            'first_name':    forms.TextInput(attrs={'class': 'mp-input', 'placeholder': 'First Name'}),
            'last_name':     forms.TextInput(attrs={'class': 'mp-input', 'placeholder': 'Last Name'}),
            'date_of_birth': forms.DateInput(attrs={'class': 'mp-input', 'type': 'date'}),
            'gender':        forms.Select(attrs={'class': 'mp-input'}),
            'blood_group':   forms.Select(attrs={'class': 'mp-input'}),
            'marital_status':forms.Select(attrs={'class': 'mp-input'}),
            'nationality':   forms.TextInput(attrs={'class': 'mp-input', 'placeholder': 'e.g. Indian'}),
            'address':       forms.Textarea(attrs={'class': 'mp-input', 'rows': 3, 'placeholder': 'Full address'}),
            'email':         forms.EmailInput(attrs={'class': 'mp-input', 'placeholder': 'Official email'}),
            'phone':         forms.TextInput(attrs={'class': 'mp-input', 'placeholder': '10-digit mobile'}),
            'personal_email':forms.EmailInput(attrs={'class': 'mp-input', 'placeholder': 'Personal email'}),
            'emergency_contact_name':  forms.TextInput(attrs={'class': 'mp-input', 'placeholder': 'Contact name & relation'}),
            'emergency_contact_phone': forms.TextInput(attrs={'class': 'mp-input', 'placeholder': '+91 XXXXX XXXXX'}),
            'reporting_manager': forms.TextInput(attrs={'class': 'mp-input', 'placeholder': 'Manager name'}),
            'work_location':     forms.TextInput(attrs={'class': 'mp-input', 'placeholder': 'e.g. Chennai – Head Office'}),
            'employment_type':   forms.TextInput(attrs={'class': 'mp-input', 'placeholder': 'e.g. Full-time'}),
    ...
    def clean_skills(self):
        """Convert 'Python:90, Django:85' shorthand into JSON for storage"""
        import json
        ...
        try:
            ...
            pass
        # Parse shorthand  Name:level, Name:level
        result = []
        for part in raw.split(','):
            ...
            if ':' in part:
                ...
                try:
                    result.append({'name': name.strip(), 'level': int(level.strip())})
                except ValueError:
                    pass
            elif part:
                result.append({'name': part, 'level': 80})
